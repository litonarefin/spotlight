(function($) {
    ...
    // Notice Hide
    $('body').on('click', '.spotlight-upgrade-popup .popup-dismiss', function(evt) {
        evt.preventDefault();
        $(this).closest('.spotlight-upgrade-popup').fadeOut(200);
    });

    // Notice Show
    $('body').on('click', '.disabled', function(evt) {
        evt.preventDefault();
        $('.spotlight-upgrade-popup').fadeIn(200);
    });
    ...
    // Search
    $('.spotlight-search-plugins #search-plugins').on('keyup', function() {
        var value = $(this).val();
...
function activatePlugin(element, file) {
    ...
    jQuery.ajax({
        url: WPSPOTLIGHTCORE.admin_ajax,
        type: "POST",
        data: {
            action: "jltwp_spotlight_recommended_activate_plugin",
            file: file,
            nonce: WPSPOTLIGHTCORE.recommended_nonce
        },
...
function installPlugin(element, plugin) {
    ...
    jQuery.ajax({
        ...
        type: "POST",
        data: {
            action: "jltwp_spotlight_recommended_upgrade_plugin",
            type: 'install',
            plugin: plugin,
            nonce: WPSPOTLIGHTCORE.recommended_nonce
        },

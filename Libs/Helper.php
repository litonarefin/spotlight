...
if ( ! class_exists( 'Helper' ) ) {
	...
	class Helper {
		 ...
		 */
		public static function jltwp_spotlight_slug_cleanup() {
			return str_replace( '-', '_', strtolower( WPSPOTLIGHT_SLUG ) );
		}
		 ...
		 */
		public static function jltwp_spotlight_current_datetime() {
			if ( function_exists( 'current_datetime' ) ) {
				return current_datetime();
			}

			return new \DateTimeImmutable( 'now', self::jltwp_spotlight_wp_timezone() );
		}

		/**
		 * Function jltwp_spotlight_wp_timezone() compability for wp version < 5.3
		 *
		 * @return DateTimeZone
		 */
		public static function jltwp_spotlight_wp_timezone() {
			if ( function_exists( 'wp_timezone' ) ) {
				return wp_timezone();
			}

			return new \DateTimeZone( self::jltwp_spotlight_wp_timezone_string() );
		}


		/**
		 * Function jltwp_spotlight_wp_timezone_string() compability for wp version < 5.3
		 *
		 * @return string
		 */
		public static function jltwp_spotlight_wp_timezone_string() {
		...
		public static function get_merged_data( $data, $start_date = '', $end_data = '' ) {
			$_data = shortcode_atts(
				array(
					'image_url'        => WPSPOTLIGHT_IMAGES . '/promo-image.png',
			...
			if ( empty( $_data['image_url'] ) ) {
				$_data['image_url'] = WPSPOTLIGHT_IMAGES . '/promo-image.png';
			}
